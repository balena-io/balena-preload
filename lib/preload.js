...
class Preloader extends EventEmitter {
	 ...
	 * must be a JSON object containing a `StatusCode` field (number / integer)
	 * and an `Error` field (string). If `StatusCode` is the number zero, the
	 * command execution is considered successful, otherwise it is assumed to
	 * have failed and the returned promise is rejected with an error message
	 * including the message provided in the `Error` field (if any).
	 *
			...
			this.stdout.once('data', (data) => {
				...
				if (result.StatusCode === 0) {
					...
					if (result.Error) {
						...
						msg.push('Result:', JSON.stringify(result, null, 4));
					}
					msg.push('');
	...
	_getState() {
		...
		return Bluebird.resolve(
			this.balena.models.device.register(this.application.id, uuid),
		)
			.then((deviceInfo) => {
